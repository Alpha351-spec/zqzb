...
test.describe('图片尺寸调整', () => {
  test.beforeAll(async () => { await ensureFixtures(); });

  test('上传图片 输入宽度800 调整 下载', async ({ page }) => {
    test.setTimeout(60000);
    await page.goto('https://zqzb-online.top/image-resize/', { waitUntil: 'domcontentloaded' });
    await page.locator('#imageInput').setInputFiles(path.join(FIXTURES, 'test.jpg'));
    await page.locator('#widthInput').fill('800');
